// This node autonomously targets and drives to a configurable offset from a non-tracked object detected using vision.
...
class DriveToObjectXYActionServer
{
protected:

  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<behavior_actions::DriveToObjectXYAction> as_; // NodeHandle instance must be created before this line. Otherwise strange error occurs.
  std::string action_name_;
  // create messages that are used to published feedback/result
  behavior_actions::DriveToObjectXYFeedback feedback_;
  behavior_actions::DriveToObjectXYResult result_;
...
public:
  ...
  DriveToObjectXYActionServer(std::string name) :
    as_(nh_, name, boost::bind(&DriveToObjectXYActionServer::executeCB, this, _1), false),
    action_name_(name),
    tf_listener_(tf_buffer_),
    orientation_command_pub_(nh_.advertise<std_msgs::Float64>("/teleop/orientation_command", 1)),
    control_effort_sub_(nh_.subscribe<std_msgs::Float64>("/teleop/orient_strafing/control_effort", 1, &DriveToObjectXYActionServer::controlEffortCB, this, ros::TransportHints().tcpNoDelay())),
    cmd_vel_sub_(nh_.subscribe<geometry_msgs::TwistStamped>("/frcrobot_jetson/swerve_drive_controller/cmd_vel_out", 1, &DriveToObjectXYActionServer::cmdVelCb, this))
  {
    const std::map<std::string, std::string> service_connection_header{ {"tcp_nodelay", "1"} };

    // /apriltag_zedx_back/tag_detection_world
    sub_ = nh_.subscribe<field_obj::Detection>("/tf_object_detection/tag_detection_world", 1, &DriveToObjectXYActionServer::update_latest_detection, this, ros::TransportHints().tcpNoDelay());

    imu_sub_ = nh_.subscribe<sensor_msgs::Imu>("/imu/zeroed_imu", 1, &DriveToObjectXYActionServer::imuCb, this, ros::TransportHints().tcpNoDelay());
  ...
  ~DriveToObjectXYActionServer(void)
  {
  }
  ...
  void update_latest_detection(const field_obj::DetectionConstPtr& msg) {
    latest_ = *msg;
  }
  ...
  std::optional<field_obj::Object> findClosestObject(const field_obj::Detection &detections, const std::string &object_id) {
    ...
    if (detections.objects.size() == 0 || (ros::Time::now() - detections.header.stamp) > ros::Duration(timeout_)) {
      ROS_ERROR_STREAM("RETURING NULLOPT, time delta is " << (ros::Time::now() - detections.header.stamp).toSec());
      ROS_ERROR_STREAM("Det size " << detections.objects.size());
      return std::nullopt;
    }
    // ROS_INFO_STREAM("BEFORE LOOP");
    for (const field_obj::Object &obj : detections.objects) {
      // print out size of points
      ROS_INFO_STREAM("Object with name " << obj.id);

      double d = dist_between_points(map_x, map_y, obj.location.x, obj.location.y);
    ...
    if (closestObject.has_value()) {
      ROS_INFO_STREAM("Closest object is " << closestObject.value().id << " at x,y " << closestObject.value().location.x << "," << closestObject.value().location.y);
    }
    else {
  ...
  void executeCB(const behavior_actions::DriveToObjectXYGoalConstPtr &goal)
  {
    x_error_ = 900;
    angle_error_ = 900;
    ...
    std::optional<field_obj::Object> closestObject;
    while (true)
    {
      r.sleep();
      ...
      closestObject = findClosestObject(latest_, goal->object_id);
    ...
    while ((abs(x_error_) > goal->x_tolerance) || (abs(y_error_) > goal->y_tolerance)) {
      ros::spinOnce(); // grab latest callback data
      if (as_.isPreemptRequested() || !ros::ok())
      {
        ROS_ERROR_STREAM("drive_to_object : Preempted");
      ...
      if (closestObject != std::nullopt) {
        ...
        object_point.point.x = closestObject->location.x;
        object_point.point.y = closestObject->location.y;
        object_point.point.z = closestObject->location.z;
      ...
      ROS_INFO_STREAM("We want to be at x,y " << goal->x_offset << "," << goal->y_offset);

      x_axis.setState(-base_link_point.point.x);
      x_axis.setCommand(-goal->x_offset);
      x_error_ = fabs(goal->x_offset - base_link_point.point.x);

      y_axis.setState(-base_link_point.point.y);
      y_axis.setCommand(-goal->y_offset);
      y_error_ = fabs(goal->y_offset - base_link_point.point.y);
      ...
      angle_error_ = fabs(msg.data - latest_yaw_);
      ...
      pid_twist.linear.x = x_eff_ * cos(field_relative_object_angle - latest_yaw_) - y_eff_ * sin(field_relative_object_angle - latest_yaw_);
      pid_twist.linear.y = x_eff_ * sin(field_relative_object_angle - latest_yaw_) + y_eff_ * cos(field_relative_object_angle - latest_yaw_);
...
{
  ros::init(argc, argv, "drive_to_object_xy");

  DriveToObjectXYActionServer driveToObjectXY("drive_to_object_xy");
  ros::spin();
