...
private:
	...
	std::unique_ptr<realtime_tools::RealtimePublisher<as726x_msgs::AS726xState> > realtime_pub_;
...
public :
			  ...
			  ros::NodeHandle                                  &controller_nh)
	{
		...
		{
			ROS_ERROR("Parameter 'publish_rate' not set in as726x controller");
			return false;
		}
		else if (publish_rate_ <= 0.0)
		{
			ROS_ERROR_STREAM("Invalid publish_rate in as726x controller (" << publish_rate_ << ")");
			return false;
		}
		interval_counter_ = std::make_unique<PeriodicIntervalCounter>(publish_rate_);

		// realtime publisher
		realtime_pub_ = std::make_unique<realtime_tools::RealtimePublisher<as726x_msgs::AS726xState>>(root_nh, "as726x_states", 2);
...
private:
	...
	{
		std::vector<std::optional<hardware_interface::as726x::AS726xState>> data;
		for (size_t i = 0; i < joint_names_.size(); i++)
		{
			auto it = std::find(msg->name.cbegin(), msg->name.cend(), joint_names_[i]);
			if (it != msg->name.cend())
			{
				const auto loc = std::distance(msg->name.cbegin(), it);
				...
				data[i]->setIndLedCurrentLimit(ind_led_current_limit);
				data[i]->setIndLedEnable(msg->ind_led_enable[loc]);
				...
				data[i]->setDrvLedCurrentLimit(drv_led_current_limit);
				...
				data[i]->setConversionType(conversion_type);
				...
				data[i]->setGain(gain);
				data[i]->setIntegrationTime(msg->integration_time[loc]);
				data[i]->setTemperature(msg->temperature[loc]);
				...
				data[i]->setRawChannelData(raw_channel_data);
				data[i]->setCalibratedChannelData(calibrated_channel_data);
			}
			else
			{
...
public:
	AS726xStateListenerController() = default;
	~AS726xStateListenerController()
	{
		sub_command_.shutdown();
	}

	bool init(hardware_interface::as726x::RemoteAS726xStateInterface *hw, ros::NodeHandle &n)
	{
