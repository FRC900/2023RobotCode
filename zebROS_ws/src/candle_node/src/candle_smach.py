...
class team_colour(smach.state):
    def __init(self):
        smach.State.__init__(self, outcomes=['green_light', 'rainbow_animation', 'emote_1', 'termination', 'white_light'], input_keys = ['allianceColor', 'auto_mode']) # Outcomes are where we can go from this state, input keys are items we get from our publishers
    ...
    def execute(self,  message):
        ...
        if auto_mode < 4: # if auto_mode 1-3,
            return 'rainbow_animation' # Go Rainbow
        elif auto_mode > 3 # if any other auto mode,
...
def main():
    ...
    rospy.Subscriber("/frcrobot_rio/match_data", MatchSpecificData, )
    rospy.Subscriber("/auto/auto_mode", AutoMode, )
