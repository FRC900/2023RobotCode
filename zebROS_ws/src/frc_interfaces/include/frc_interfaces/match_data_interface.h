...
class MatchHWState
{
	public:
        ...
        AllianceColor getAllianceColor(void)           const {return alliance_color_;}
        ...
        void setMatchTimeRemaining(const double match_time_remaining)            {match_time_remaining_ = match_time_remaining;}
        ...
        void setAllianceColor(const AllianceColor alliance_color)                {alliance_color_ = alliance_color;}
        void setMatchType(const int match_type)                                  {match_type_ = match_type;}
        void setDriverStationLocation(const int driver_station_location)         {driver_station_location_ = driver_station_location;}
        void setMatchNumber(const int match_number)                              {match_number_ = match_number;}
        void setReplayNumber(const int replay_number)                            {replay_number_ = replay_number;}
                                                                              
        void setEnabled(const bool enabled)                                      {enabled_ = enabled;}
        void setDisabled(const bool disabled)                                    {disabled_ = disabled;}
        void setAutonomous(const bool autonomous)                                {autonomous_ = autonomous;}
        void setFMSAttached(const bool fms_attached)                             {fms_attached_ = fms_attached;}
        void setDSAttached(const bool ds_attached)                               {ds_attached_ = ds_attached;}
        void setOperatorControl(const bool operator_control)                     {operator_control_ = operator_control;}
        void setTest(const bool test)                                            {test_ = test;}
        void setEStopped(const bool e_stopped)                                   {e_stopped_ = e_stopped; }

        void setBatteryVoltage(const double battery_voltage)                     {battery_voltage_ = battery_voltage;}
	...
	private:
		...
		AllianceColor  alliance_color_{0};
		int            match_type_{0};
