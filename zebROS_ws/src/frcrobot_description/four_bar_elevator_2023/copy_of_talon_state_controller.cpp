...
private:
    ...
    std::unique_ptr<realtime_tools::RealtimePublisher<sensor_msgs::JointState> > realtime_state_pub_; //added by scarlett
...
public:
			  ...
			  ros::NodeHandle                         &controller_nh)
	{
		...
		for (size_t i = 0; i < num_hw_joints_; i++)
		{
        	...
        	realtime_pub_ = std::make_unique<realtime_tools::RealtimePublisher<sensor_msgs::JointState>>(root_nh, "talon_joint_states", 4); //added by scarlett
		
        	auto &m = realtime_joint_pub_->msg_;
		// get joints and allocate message
		for (size_t i = 0; i < num_hw_joints_; i++) 
		{
			m.name.push_back(joint_names[i]);
	...
	void update(const ros::Time &time, const ros::Duration &period)
	{
		...
		if (interval_counter_->update(period))
		{
			...
			if (realtime_pub_->trylock())
			{
				...
				auto &m = realtime_state_pub_->msg_;
				m.header.stamp = time;
				for (size_t i = 0; i < num_hw_joints_; i++)
				{
					auto &ts = talon_state_[i];
