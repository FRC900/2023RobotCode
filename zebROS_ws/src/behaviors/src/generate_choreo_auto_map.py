...
if len(sys.argv) < 2:
    print("Usage: generate_choreo_auto_map.py [path_to_chor_file]")
    sys.exit(0)
...
for path_key in paths:
    ...
    for t in trajectory:
        ...
        csv += f"{t['timestamp']},{untransformed_pose.pose.position.x},{untransformed_pose.pose.position.y},{transformations.euler_from_quaternion([untransformed_pose.pose.orientation.x, untransformed_pose.pose.orientation.y, untransformed_pose.pose.orientation.z, untransformed_pose.pose.orientation.w])[2]},{t['angularVelocity']},{t['velocityX']},{t['velocityY']},{idx}\n"
    
    with open(f"/home/ubuntu/2023RobotCode/zebROS_ws/src/behaviors/path/map_rel_{path_key}_csv.csv", "w") as csv_file:
        print(csv)
        csv_file.write(csv)
