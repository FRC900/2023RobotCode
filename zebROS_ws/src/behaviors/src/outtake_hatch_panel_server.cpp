			...
			ae_("move_elevator_server", true)
	{
		GoalDetectSub_ = nh_.subscribe("goal_detect_msg",1, &OuttakeHatchPanelAction::goalDetectCallback, this) //TODO make sure this is linked up correctly
			/* std::map<std::string, std::string> service_connection_header;
			   ...
			   IntakeSrv_ = nh_.serviceClient<elevator_controller::Intake>("/frcrobot/elevator_controller/outtake", false, service_connection_header);
		...
		void executeCB(const behaviors::PlaceGoalConstPtr &goal)
		{
			...
			elev_goal.setpoint_index = INTAKE //TODO
		...
		//send command to panel_outtake_controller to grab the panel ---------------------------------------
