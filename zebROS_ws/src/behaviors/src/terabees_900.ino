/*
 * This code allows you to check if the I2C communication between the TeraRanger and the Arduino board works.
 * You just need to upload the code to the board with the TeraRanger connected on pins SDA, SCL, GND and 5V.
 * If the supply of the TeraRanger is stopped or interrupted while the code is running, just press the reset button on the board.
 * There are two way to check if the communication works:
 * - the first is to open the serial monitor and check if the distance is printed like this: "Distance in mm: XXX";
 * - the second is to observe the LED "L" on the board, the brightness of the LED will change with the distance measured by the TeraRanger One (Range: 20cm to 1m).
 * If the communication works you can remove the ERASABLE part of the code shown below.
 * Warning: Make sure that you connect the right pins of the TeraRanger One to your Arduino, for I2C operation you need to modify the open ended cable provided in the box.
 * Default I2C base address of the TeraRanger one is 0x30.
 * 
 * This is using this framework: https://arduino-pico.readthedocs.io/en/latest/
 * Waveshare pico board needs to be updated 
 *
 * Frequency is 400Khz on I2C
 * There are Built-in pull-up resistors (on SCL/SDA) so avoid additional pull-up resistors on the same bus to prevent transmission problems)
 */
 

#include <Wire.h>
// For TeraRanger Evo
#define SENSOR_ADDR 0x31  

// Create a Cyclic Redundancy Checks table used in the "crc8" function
static const uint8_t crc_table[] = {
    
};

/*
 * Brief : Calculate a Cyclic Redundancy Checks of 8 bits
 * Param1 : (*p) pointer to receive buffer
 * Param2 : (len) number of bytes returned by the TeraRanger
 * Return : (crc & 0xFF) checksum calculated locally
 */
uint8_t crc8(uint8_t *p, uint8_t len) {
  uint8_t i;
  uint8_t crc = 0x0;
  while (len--) {
    i = (crc ^ *p++) & 0xFF;
    crc = (crc_table[i] ^ (crc << 8)) & 0xFF;
  }
  return crc & 0xFF;
}

uint8_t buf0[3];           // The variable "buf[3]" will contain the frame sent by the TeraRanger
uint8_t buf1[3];           // The variable "buf[3]" will contain the frame sent by the TeraRanger
uint16_t distance0 = 0;    // The variable "distance" will contain the distance value in millimeter
uint16_t distance1 = 0;    // The variable "distance" will contain the distance value in millimeter
uint8_t CRC0 = 0;          // The variable "CRC" will contain the checksum to compare at TeraRanger's one
uint8_t CRC1 = 0;          // The variable "CRC" will contain the checksum to compare at TeraRanger's one

const byte short_mode[2] = {0x02,0x01};
const byte long_mode[2] = {0x02,0x03};

int inter = 25; // Inter reading delay, replace by 50 if using long mode

void setupWire0(){
  Wire.setSDA(4);
  Wire.setSCL(5);
  Wire.begin();           // Join I2C bus as master
  // Set your sensor mode
  Wire.beginTransmission(SENSOR_ADDR);
  //const byte short_mode[2] = {0x02,0x01};
  //const byte long_mode[2] = {0x02,0x03};
  Wire.write(short_mode, 2);
  //Wire.write(long_mode, 2); // Sensor is in long mode when powered up
  Wire.endTransmission();
  delay(25);                  // This delay is necessary to ensure mode is switched correctly
}

void setupWire1(){
  //This sets the second I2C bus up.
  Wire1.setSDA(2);
  Wire1.setSCL(3);
  Wire1.begin();           // Join I2C bus as master
  // Set your sensor mode
  Wire1.beginTransmission(SENSOR_ADDR);
  Wire1.write(short_mode, 2);
  //Wire.write(long_mode, 2); // Sensor is in long mode when powered up
  Wire1.endTransmission();
  delay(25);                  // This delay is necessary to ensure mode is switched correctly
}

void setup() {
  setupWire0();
  setupWire1();
  Serial.begin(921600);
}

// The main loop starts here
void loop() {
  
  Wire.beginTransmission(SENSOR_ADDR);  // Transmit to Evo Mini (THIS IS THE I2C BASE ADDRESS, CHANGE HERE IN CASE IT IS DIFFERENT)
  Wire.write(0x00);                     // Sends measure trigger byte
  Wire.endTransmission();               // Stop transmitting

  Wire1.beginTransmission(SENSOR_ADDR);  // Transmit to Evo Mini (THIS IS THE I2C BASE ADDRESS, CHANGE HERE IN CASE IT IS DIFFERENT)
  Wire1.write(0x00);                     // Sends measure trigger byte
  Wire1.endTransmission();               // Stop transmitting
  
  Wire.requestFrom(SENSOR_ADDR, 3);     // Read back three bytes from Evo Mini (THIS IS THE I2C BASE ADDRESS, CHANGE HERE IN CASE IT IS DIFFERENT)
  buf0[0] = Wire.read();                 // First byte of distance
  buf0[1] = Wire.read();                 // Second byte of distance
  buf0[2] = Wire.read();                 // Byte of checksum

  Wire1.requestFrom(SENSOR_ADDR, 3);     // Read back three bytes from Evo Mini (THIS IS THE I2C BASE ADDRESS, CHANGE HERE IN CASE IT IS DIFFERENT)
  buf1[0] = Wire1.read();                 // First byte of distance
  buf1[1] = Wire1.read();                 // Second byte of distance
  buf1[2] = Wire1.read();                 // Byte of checksum
  
  CRC0 = crc8(buf0, 2);                   // Save the "return" checksum in variable "CRC" to compare with the one sent by the TeraRanger
  CRC1 = crc8(buf1, 2);                   // Save the "return" checksum in variable "CRC" to compare with the one sent by the TeraRanger
  
  if (CRC0 == buf0[2] && CRC1 == buf1[2]) {
    distance0 = (buf0[0]<<8) + buf0[1];    // Calculate distance in mm
    Serial.print(distance1);
    distance1 = (buf1[0]<<8) + buf1[1];    // Calculate distance in mm
    Serial.print(",");
    Serial.println(distance0);
  }
  else {
    Serial.println("!!!!!!!!!!!!!!!CRC error!!!!!!!!!!!!!!!");
  }

  delay(inter);                         // This delay is necessary to prevent reading too many times the same measurement (To be adapted depending on the sensor mode)
}