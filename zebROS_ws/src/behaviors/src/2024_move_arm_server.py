...
class ArmAction(): # Creates ArmAction class
    ...
    def callback(self, data):
        if self.arm_index == None: # See how we start using arm_index here? It'll be more important why later
            for i in range(len(data.name)): # Starts searching through the names of the data
                if data.name[i] == "arm": # Pretty much looking for the name of the motor that controls the arm (We named it 'arm' lol)
    ...
    def execute_cb(self, goal):
        ...
        rospy.loginfo(f"{self._action_name}: Executing. Moving arm to {goal}") # This will give info to the person running the server of whats going on
