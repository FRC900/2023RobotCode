    ...
    def execute_cb(self, goal):
        ...
        while True:
            ...
            if self.server.is_preempt_requested():
                self.left_joint_shooter_pub.publish(std_msgs.msg.Float64(0))
                self.right_joint_shooter_pub.publish(std_msgs.msg.Float64(0))
                self.server.set_preempted()
                break

            elif ((left_percent_difference < self.tolerance) and (right_percent_difference < self.tolerance)):
                self._result.success = True
                self._feedback.left_percent_complete = 100.0
                self._feedback.right_percent_complete = 100.0
                self._feedback.is_shooting_at_speed = True
