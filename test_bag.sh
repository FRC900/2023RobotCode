	...
	then
		...
		then
			...
			then
				...
				cp $HOME/2018Offseason/$practice_merge_file $HOME/2018Offseason/unmerged_${practice_merge_file}
				~/2018Offseason/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py $practice_merge_file $var unmerged_$practice_merge_file -v
		...
		then 
			...
			then
				...
				cp $var $HOME/2018Offseason/match0$(basename $var)
				continue
			fi
			...
			then 
				...
				~/2018Offseason/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py ${bag_name}_${count}.bag $var ${bag_name}*.bag -v
