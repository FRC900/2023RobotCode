	...
	then
		...
		then
			...
			then
				...
				cp $HOME/2019RobotCode/$practice_merge_file $HOME/2019RobotCode/unmerged_${practice_merge_file}
				~/2019RobotCode/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py $practice_merge_file $var unmerged_$practice_merge_file -v
		...
		then 
			...
			then
				...
				cp $var $HOME/2019RobotCode/match0$(basename $var)
				continue
			fi
